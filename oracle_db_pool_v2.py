...
class DatabaseService:
    ...
    async def initialize_pool(self, user: str, password: str, dsn: str, 
        ...
        try:
            ...
            # Sync pool for pandas queries
            self.sync_pool = oracledb.create_pool(
                user=user,
                password=password,
                dsn=dsn,
                min=min_connections,
            ...
            logger.info(f"Connection pools initialized with {min_connections}-{max_connections} connections")
    ...
    def get_sync_connection(self):
        """Get sync connection from pool for pandas"""
        if not self.sync_pool:
            raise Exception("Sync connection pool not initialized")
        return self.sync_pool.acquire()
...
async def acquire_connection_node(state: AgentState) -> AgentState:
    ...
    try:
        # Get async connection for raw queries
        async_connection = await db_service.get_connection()
        # Get sync connection for pandas queries
        sync_connection = db_service.get_sync_connection()
        
        # Store both connections
        state['db_connection'] = {
            'async': async_connection,
            'sync': sync_connection
        }
        logger.info("Database connections acquired for agent execution")
    except Exception as e:
        error_msg = f"Failed to acquire database connections: {str(e)}"
...
async def pandas_query_node(state: AgentState) -> AgentState:
    """Execute pandas query using sync connection"""
    if state.get('error') or not state.get('db_connection'):
        logger.warning("Skipping pandas query due to connection error")
        return state
    
    try:
        # Use sync connection for pandas
        sync_connection = state['db_connection']['sync']
        
        # Execute query using pandas in thread pool to avoid blocking
        loop = asyncio.get_event_loop()
        with concurrent.futures.ThreadPoolExecutor() as executor:
            query = "SELECT user_id, username, email FROM users WHERE user_id = :user_id"
            df = await loop.run_in_executor(
                executor, 
                lambda: pd.read_sql(query, sync_connection, params={'user_id': state['user_id']})
            )
...
async def raw_query_node(state: AgentState) -> AgentState:
    """Execute raw query using async connection"""
    if state.get('error') or not state.get('db_connection'):
        logger.warning("Skipping raw query due to connection error")
        return state
    
    try:
        # Use async connection for raw queries
        async_connection = state['db_connection']['async']
        ...
        # Rollback on error
        try:
            await state['db_connection']['async'].rollback()
...
async def multiple_operations_node(state: AgentState) -> AgentState:
    """Execute multiple database operations using both connections"""
    if state.get('error') or not state.get('db_connection'):
    ...
    try:
        ...
        # Operation 2: Pandas query for analysis (in thread pool)
        loop = asyncio.get_event_loop()
        with concurrent.futures.ThreadPoolExecutor() as executor:
            analytics_query = """
            ...
            """
            analytics_df = await loop.run_in_executor(
                executor,
                lambda: pd.read_sql(analytics_query, sync_connection, params={'user_id': state['user_id']})
            )
        ...
        logger.info("Multiple operations executed successfully")
        
    except Exception as e:
        error_msg = f"Multiple operations failed: {str(e)}"
        logger.error(error_msg)
        state['error'] = error_msg
        # Rollback async operations
        try:
            await state['db_connection']['async'].rollback()
            logger.info("Async operations rolled back due to error")
        except:
            pass
...
async def release_connection_node(state: AgentState) -> AgentState:
    """Final node: Release both database connections back to pools"""
    try:
        if state.get('db_connection'):
            connections = state['db_connection']
            
            # Release async connection
            if connections.get('async'):
                await connections['async'].close()
            
            # Release sync connection
            if connections.get('sync'):
                connections['sync'].close()
            
            state['db_connection'] = None
            logger.info("Database connections released back to pools")
    except Exception as e:
        logger.error(f"Error releasing connections: {e}")
...
class AgentDatabaseContext:
    """Context manager for entire agent execution with both connection types"""
    ...
    async def __aenter__(self):
        self.async_connection = await self.db_service.get_connection()
        self.sync_connection = self.db_service.get_sync_connection()
        logger.info("Database connections acquired for agent context")
        return {
            'async': self.async_connection,
            'sync': self.sync_connection
        }
    
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        try:
            if exc_type and self.async_connection:
                await self.async_connection.rollback()
                logger.info("Async transaction rolled back due to exception")
            ...
            logger.info("Database connections released from agent context")
        except Exception as e:
...
async def execute_agent_with_context(user_id: str):
    ...
    try:
        async with AgentDatabaseContext(db_service) as connections:
            async_conn = connections['async']
            sync_conn = connections['sync']
            
            # Pandas query (in thread pool)
            loop = asyncio.get_event_loop()
            with concurrent.futures.ThreadPoolExecutor() as executor:
                user_query = "SELECT user_id, username, email FROM users WHERE user_id = :user_id"
                user_df = await loop.run_in_executor(
                    executor,
                    lambda: pd.read_sql(user_query, sync_conn, params={'user_id': user_id})
                )
            ...
            # Another pandas query (in thread pool)
            with concurrent.futures.ThreadPoolExecutor() as executor:
                activity_query = "SELECT * FROM user_activity WHERE user_id = :user_id ORDER BY activity_date DESC"
                activity_df = await loop.run_in_executor(
                    executor,
                    lambda: pd.read_sql(activity_query, sync_conn, params={'user_id': user_id})
                )
